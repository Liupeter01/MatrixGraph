...
int CalculateEnVertex(MGraph G, VertexType x)		//计算某一个顶点的入度数量
{
          ...
          if (x_pos != -1)
          {
                    ...
                    {
                              if (G.Edge[i][LocateVertex(G, x)] != 0 && G.Edge[i][LocateVertex(G, x)] != INFINITYSIZE)      //以列为单位进行遍历
                              {
                                        counter++;
                              }
                    }
...
int CalculateOutVertex(MGraph G, VertexType x)			//计算某一个顶点的出度数量
{
          ...
          if (x_pos != -1)
          {
                    ...
                    for (int i = 0; i < G.vexnum; ++i)
                    {
                              if (G.Edge[LocateVertex(G, x)][i] != 0 && G.Edge[LocateVertex(G, x)][i] != INFINITYSIZE)      //以列为单位进行遍历
                              {
                                        counter++;
                              }
                    }
